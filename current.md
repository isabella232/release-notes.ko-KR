...
Adobe ID를 사용하여 제품, 지역 및 이벤트 기본 설정에 따라 상태 이벤트 알림을 구독할 수 있습니다.
...
| 개인화된 신속한 이메일 전달 | <ul><li>이벤트 알림은 CSO 및 CMR이 열리거나 업데이트되거나 닫히는 즉시 전송됩니다.</li><li>구성된 기본 설정과 일치하는 관련 이벤트 알림만 받습니다.</li><li>계정 기본 설정에 구성된 언어를 기반으로 지역화된 알림을 받았습니다.</li></ul> |
| 개인화된 제품 내 알림 | 알림 환경 설정 및 제품 권한에 맞는 이벤트가 공지 패널에 표시됩니다. |
