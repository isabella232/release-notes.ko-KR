...
## [!UICONTROL 경험 플랫폼] 및 관리 {#platform}

[!UICONTROL Experience Platform], Experience Cloud 인터페이스, 제품 관리, Experience Platform Launch, Identity Service 및 보안 게시판을 위한 릴리스 정보
...
| 기능 | 설명 |
| -----------| ---------- |  
| Samesite 쿠키 설정 지원 | Samesite 쿠키 설정이 Analytics에서 설정된 모든 쿠키 세트에 추가됩니다. 이 변경 사항을 사용하면 Samesite 쿠키 필드를 요구하는 Chrome 변경 사항을 준수할 수 있습니다. |
| 작업 영역: 드롭다운 필터에 대한 항목 제한을 50에서 200로 증가 | 드롭다운 필터에 배치할 수 있는 항목의 제한이 50 개에서 200 개로 증가했습니다. 이 개선 사항은 모든 국가 (195) 를 필터에 추가하거나 모든 미국 주 및 지방 (52 개) 를 추가하는 등 다양한 사용 사례를 수용합니다. |

#### 수정 사항

* 전체 화면 모드에서 실시간 보고서의 텍스트 표시 문제를 해결했습니다. (AN-183168)
...
[!UICONTROL 2019 년 8 월 8 일 Appmeasurement] 2.16.0 릴리스
...
| 방문자 ID 서비스 4.4.0 | 방문자 ID 서비스 4.4.0를 사용하도록 appmeasurement를 업그레이드했습니다. |

#### 수정 사항

* Isreadytotrack 이 true 이기 전에 발생한 appmeasurement 큐에 버그가 수정되었습니다.
...
* 이제 관리 탭이 관리 권한이 있는 사용자 계정에만 표시됩니다 (AAM -48557).
* 이제 목록 사용자 API가 전체 사용자 세부 사항을 반환합니다 (AAM -48662).
* 이제 트레이트 폴더 목록 크기를 조정할 수 있습니다 (AAM -48800).
* 여러 UI 액세서빌러티 최적화 (AAM -48865, AAM -48933).
* 관리 및 Data Sources 페이지에 대한 최적화 최적화 (AAM -48514).
...
[!UICONTROL 제어판에서] 호스팅되는 Adobe Campaign Classic 및 Adobe Campaign Standard 고객은 제어판을 사용할 수 있습니다. 제어판에 대한 업그레이드는 필요하지 않습니다.
