...
* [Experience Cloud 코어 서비스](#core-services)
...
* &quot; 좋아요 &quot;절의 올바른 사용을 위한 [!UICONTROL 트리거] 정의의 문제를 수정했습니다. (MCUI-6611)
...
| 기능 | 설명 |
| -----------| ---------- |  
| 고유 개수에 대한 세그멘테이션 | 이제 차원 내에서 항목의 고유 개수를 세그먼트화할 수 있습니다. 예: &quot;5개 이상의 고유 제품을 본 방문자&quot; 또는 &quot;5개 이상의 고유 페이지를 본 방문 위치&quot; (차원의 고유 카운트 (예: 고객 수, 제품 # 수 등) 인 지표를 만들려면 계산된 지표에서 대략적인 카운트 수 기능을 계속 사용할 수 있습니다.) |
| Java 11에 대한 애드혹 분석 지원 | Ad Hoc Analysis는 이제 Java 11과 호환됩니다. Java 11 사용에 대한 몇 가지 주의해야 할 사항이 있습니다. |
...
| Analysis Workspace에서 CSV 다운로드로 업데이트 | 2019년 4월 10일 | 2019년 4월 11일부터, Analysis Workspace의 CSV 다운로드(및 클립보드로 복사)가 변경되어 내보낸 데이터의 형식을 제거할 수 있습니다.  <ul><li>천 단위 구분 기호가 더 이상 포함되지 않습니다. 소수점 구분 기호는 계속해서 포함되며, **구성 요소** &gt; **보고서 설정** &gt; **천 단위 구분 기호**에 정의된 형식을 따릅니다. 참고: 쉼표를 소수점 구분 기호로 사용하는 숫자 값은 내보낸 CSV에서 계속 따옴표로 표시됩니다.</li><li>통화 기호가 표시되지 않습니다.</li><li>퍼센트 기호가 표시되지 않습니다. 백분율은 10진수 형식입니다. 예를 들어, 75%는 0.75로 표시됩니다.</li><li>시간은 초 단위로 표시됩니다.</li><li>집단 테이블에는 원시 값만 표시됩니다. 백분율은 제거됩니다.</li><li>숫자가 올바르지 않으면 빈 셀이 표시됩니다.</li></ul> |
| Analysis Workspace 디버거 명령 변경 예정 | 2019년 4월 4일 | 분석 작업 공간 디버거를 켜는 콘솔 명령은 2019 **년 6 월 13 일에 Adobetools. debug. includeoberonxml로 변경됩니다**. |
...
| Analysis Workspace에서 CSV 다운로드로 업데이트 | 2019년 1월 9일 | 2019년 2월 7일부터 Analysis Workspace에서 CSV 다운로드(및 클립보드에 복사) 항목에는 더 이상 천단위 구분 기호가 포함되지 않습니다. 참고: Analysis Workspace UI는 천단위 구분 기호를 계속 표시합니다. 또한 소수 구분 기호는 계속 포함될 것이며 **[!UICONTROL 구성 요소]** &gt; **[!UICONTROL 보고서 설정]** &gt; **[!UICONTROL 천 단위 구분 문자로 정의된 형식을 준수합니다]**. |
...
* 이제 규칙 기반, 세그먼트 또는 온보딩과 다른 트레이트 유형을 기준선으로 사용하여 [!UICONTROL 알고리즘 모델을] 만들려고 할 때 오류 메시지가 표시됩니다. 오류 메시지: &quot;규칙 기반 트레이트, 온보드 트레이트 및 세그먼트만 기준으로 사용할 수 있습니다.&quot; (AAM-45235).
* [!UICONTROL 라이프타임] 지표는 [!UICONTROL 트레이트] 및 [!UICONTROL 폴더 트레이트레이트] 페이지에서 제거되었습니다.
* 세그먼트 매핑 시작 및 종료 날짜가 이제 [!UICONTROL 대상 일반 보고서]에 포함되어 있습니다(AAM-44997).

### 수정 사항

* 일부 상황에서 [!UICONTROL 아웃바운드 파일 내역 보고서가] 로드되지 않는 문제를 해결했습니다 (AAM -45713).
* 빈 필드에서 트레이트 추가를 클릭하면 이전에 선택한 트레이트가 [!UICONTROL 세그먼트 빌더에] 추가되는 [!UICONTROL 문제를] 해결했습니다 (AAM -45599).
* 사용자가 제외된 트레이트가 포함된 알고리즘 모델을 편집하거나 비활성화할 수 없는 문제가 해결되었습니다(AAM-45552).

## Advertising Cloud {#adcloud}

4 월 15 일 2019 년 4 월 13 일에 업데이트됨

| 기능 | 설명 |
| -----------| ---------- | 
| 캠페인 검색 | 이제 날짜 범위 선택기에서 달력이 일요일부터 토요일까지 배열됩니다. 이전에는 일요일부터 토요일까지 배열되었습니다. |
| 보고서 | 포트폴리오 보고서에는 이제 선택적 열 _포트폴리오 비용 전략이 포함됩니다_. |
...
| 기능 | 설명 |
| -----------| ---------- |  
| 모바일 시각적 경험 작성기 | 기본 모바일 앱용 VEC(시각적 경험 작성기)를 사용하면 연속적인 개발 종속성 및 앱 릴리스 주기 없이 자체적인 방식으로 기본 모바일 앱에 대한 활동을 만들고 컨텐츠를 개인화할 수 있습니다. |
...
2019 년 3 월 14 일 발표된 AEM 6.3 SP 3-CFP 3 (6.3.3.3) 는 2017 년 4 월 AEM 6.3의 일반 출시 이후에 출시된 주요 고객의 주요 업데이트입니다.
