...
>[!IMPORTANT]
>
>Marketing Cloud ID 서비스 라이브러리 1.9.0+를 사용하면 기본적으로 Adobe Media Optimizer에 대한 ID 동기화가 시작됩니다. [ID 동기화 및 일치율 이해](https://marketing.adobe.com/resources/help/ko_KR/mcvid/mcvid-match-rates.html)를 참조하십시오.
...
* **(업데이트 날짜: 10/28, 11/4)** ID 서비스는 [!DNL  Marketing Cloud]ID(MID)를[!DNL  Adobe Media Optimizer] 페이지의 및 기타 내부 [!DNL  Adobe] 도메인과 직접 자동으로 동기화합니다. 기존 계정 및 신규 계정 전체에 자동 동기화가 활성화됩니다. 이것은 [!DNL  Media Optimizer]에 대한 일치율을 개선하는 데 도움이 됩니다. VisitorAPI.js 버전 1.9, 이상에 적용됩니다. [ID 동기화 및 일치율 이해](https://marketing.adobe.com/resources/help/ko_KR/mcvid/mcvid-match-rates.html)를 참조하십시오. (MCID-152, MCID-184)
...
자세한 내용은 Developer Connection에서 [Permission API Changes for October](https://marketing.adobe.com/developer/forum/administration/permissions-api-changes-for-october)를 참조하십시오.
...
* 오류 샘플링을 통해 인바운드 데이터 파일에서 오류가 반환되지 않던 [!UICONTROL 온보딩 상태 보고서]의 버그를 수정했습니다. 또한, [온보딩 상태 보고서: 정보](https://marketing.adobe.com/resources/help/ko_KR/aam/onboarding-status-report.html)를 참조하십시오. (AAM-26460)
...
* DIL은 ID를 페이지에서 바로 Adobe Media Optimizer 및 다른 내부 Adobe 도메인과 동기화합니다. 이 기능을 활성화하기 위해 DIL에서는 새 쿠키인 ` AAM_ *` partner name here`*_ *` containerNSID here`*`를 설정합니다. ` *`파트너 이름`*` 및 ` *`containerNSID`*` 변수에 대한 정보는 [DIL 생성](https://marketing.adobe.com/resources/help/ko_KR/aam/r_dil_create.html)을 참조하십시오. 이 유형의 동기화는 Media Optimizer에 대한 일치율을 개선하는 데 도움이 됩니다. (MCID-138)
* DIL에서는 ` loadTimeout` ID 서비스의 [!DNL  Marketing Cloud] 속성으로 설정한 값을 사용하여 데이터 수집 서버의 ` getMarketingCloudVisitorID` 응답을 기다리는 시간을 결정합니다. [loadTimeout](https://marketing.adobe.com/resources/help/ko_KR/mcvid/mcvid-loadtimeout.html)을 참조하십시오. (MCID-143)

예정된 릴리스 날짜: **2016년 10월 25일**

**개선 사항**

* 이제 Facebook 브랜드 콘텐츠가 지원됩니다. 고유한 Facebook 페이지 브랜드가 아닌 타사 브랜드 콘텐츠를 게시할 때 어디서나 게시에서 게시에 주요 타사에 태그를 지정할 수 있습니다. 자세한 내용은 Facebook 브랜디드 콘텐츠(Branded Contents) 정책을 참조하십시오.[!DNL  Publish Anywhere] 자세한 내용은 [Facebook 브랜드 콘텐츠 정책](https://www.facebook.com/policies/brandedcontent/)을 참조하십시오.
...
[Adobe Media Optimizer](https://forums.adobe.com/community/experience-cloud/advertising-cloud) 커뮤니티를 소개하게 되어 기쁘게 생각합니다! Adobe Media Optimizer는 질문을 하고, 조언을 받고, 우수 사례를 공유하고, 제품에 대해 더 많이 학습할 수 있는 장소입니다. 이 커뮤니티는 전 세계 Adobe 팀 및 동료 제품 사용자와 연결, 공유 및 공동 작업할 플랫폼을 제공합니다. 도움을 받고 다른 사용자를 지원할 수 있는 좋은 기회입니다.
