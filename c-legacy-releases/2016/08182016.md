...
* ID 서비스에서 방문자 [!DNL  Marketing Cloud] ID 클라이언트측 또는 서버측을 생성했는지 또는 ID 호출이 시간 초과되었는지 확인하는 함수가 visitorAPI.js 코드에 대한 추가되었습니다. [시간 초과 추적 기능](https://marketing.adobe.com/resources/help/en_US/mcvid/mcvid-timeout-functions.html) 및 [클라이언트측 방문자 ID 생성 추적](https://marketing.adobe.com/resources/help/en_US/mcvid/mcvid-client-side-id.html)을 참조하십시오. (MCID-116)
...
* CORS(교차 도메인 리소스 공유)에 대한 DIL 지원이 추가되었습니다. [useCorsOnly](https://marketing.adobe.com/resources/help/en_US/aam/dil-use-cors-only.html)를 참조하십시오. (AAM-25319, AAM-20139).
...
* 이제 DIL은 기본적으로 HTTPS 호출을 안전하게 만듭니다. 같은 페이지에서 [!DNL  Audience Manager] DIL 코드와 visitorAPI.js 코드를 사용하는 고객은 DIL 변수 ` secureDataCollection= false`를 설정해야 합니다. [secureDataCollection](https://marketing.adobe.com/resources/help/en_US/aam/dil-secure-data-collection.html)을 참조하십시오.
