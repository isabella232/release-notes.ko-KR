...
* 푸시 테스트 기능을 사용할 때 푸시 알림 컨텐츠 사용 가능 플래그가 항상 *`true`로 설정되어 있는 문제가 해결되었습니다.*
* [!UICONTROL 모바일] 서비스 관련 URL를 [!DNL *.marketing.adobe.com] 에서 [!DNL *.experiencecloud.adobe.com]로 업데이트했습니다.
...
이제 패널 놓기 영역에 드롭다운 기능이 있습니다. 드롭다운 기능을 통해 최종 사용자가 제어된 방식으로 프로젝트의 데이터와 상호 작용할 수 있습니다. 여러 구성 요소(또는 차원 항목)를 끌어 놓은 다음 드롭다운에서 전환하여 패널 컨텐츠를 필터링할 수 있습니다. 또한 동일한 패널에서 여러 개의 드롭다운을 생성할 수도 있습니다.

예를 들어 국가별 보고를 제공하기 위한 여러 버전의 프로젝트/패널이 있다고 가정해봅시다. 이제 이러한 프로젝트/패널을 단일 패널로 축소하고 두 개의 데이터 세트 간에 필터링하는 대신 국가 드롭다운에 추가할 수 있습니다.

[색상 팔레트](https://marketing.adobe.com/resources/help/en_US/analytics/analysis-workspace/color_palettes.html)
...
[오디오 분석](https://marketing.adobe.com/resources/help/en_US/sc/appmeasurement/hbvideo/media-workspace-templates.html)을 참조하십시오.
...
Adobe Analytics 구현으로 검색 엔진 데이터의 자동화된 추적에 대해 [자동 추적 모드](https://marketing.adobe.com/resources/help/en_US/analytics/advertising/aa_manual_vs_automatic_tracking.html)가 도입되었습니다.

서버 호출 사용량 개선

서버 호출 사용량이 약정 수준에 근접하면 사전 알림을 제공하는 새로운 경고 유형 _[서버 호출](https://marketing.adobe.com/resources/help/en_US/analytics/servercall/scu_alerts.html)_을 추가했습니다.
...
이 기능을 통해 기업이 청취자의 도달 범위를 측정하고, 컨텐츠의 인기를 측정하며, 사용자의 행동과 유사 모델링을 기반으로 사용자의 참여를 유지합니다. 고객은 사용자 정의 가능한 대시보드를 작성하여 오디오 스트리밍 컨텐츠 전달 및 성능을 확인하고, 현재 트렌드를 파악하며, 컨텐츠를 통해 청취자의 흐름을 시각화하여 신속하고 적절한 맞춤형 결정을 내릴 수 있습니다.

이점:

* 오디오 컨텐츠 상위 및 하위 실적을 모니터링하여 트렌드를 확인하고, 오디오 컨텐츠를 보다 효율적으로 작성, 분배, 맞춤설정 및 마케팅하는 방법을 판별합니다.
* True 대상 비율 - 도달 범위를 측정하기 위해 과거 및 현재 데이터를 벤치마킹합니다. 또한 집단 분석을 통해 UV 및 사용자 측정항목, 청취 참여에 계층화를 적용하고, 청취자를 반환합니다.
* 고객 여정 흐름 - 자동차를 포함한 모든 청취 장비에서 사용자를 측정하여 장비 간 청취 경험, 분당 동시 청취자 및 청취자가 다음에 액세스하는 컨텐츠를 파악합니다.
* 수익 창출 전달 - 실용적인 실시간 인사이트와 데이터 예외 항목을 실현하여 정보에 입각한 비즈니스 의사 결정을 내릴 수 있습니다. 예를 들어, 다음에 프로그래밍할 컨텐츠, 개인화된 청취 환경을 제공하는 방법, 구독 업그레이드 요청 시기, 오디오 광고에 대해 더 높은 CPM 비율을 활성화하는 방법 등이 있습니다.
* 세분화된 고급 측정 단위 - 업계 최고의 SDK를 사용하여 10초 오디오 컨텐츠 및 1초 스트리밍 광고를 측정하여 경험의 품질(중지, 오류, 시작 시간)과 컨텐츠 및 광고 KPI를 평가할 수 있습니다.

**컨텐츠 추적 API 다운로드**

웹 연결에 관계없이 장치에서 다운로드 및 재생되는 미디어 컨텐츠 추적(비디오 및 오디오)에 대한 지원을 제공합니다. 이 기능은 Media Collection API 위에 구축되며 동일한 플레이어 추적 사양을 따릅니다. 현재 SDK 지원은 없습니다.
...
다음 플랫폼에 대한 [AppMeasurement](https://marketing.adobe.com/resources/help/en_US/sc/appmeasurement/release/index.html) 릴리스 내역이 필요하면 AppMeasurement 릴리스 내역을 참조하십시오.
...
>[!IMPORTANT]
>
> [Report Builder](https://marketing.adobe.com/resources/help/en_US/arb/t_install_arb.html)의 설치를 최신 버전으로 업데이트합니다. 이 업데이트는 2018년 4월부터 시작되는 Admin Console로 Analytics 사용자 ID 이전 실행을 위한 전제 조건입니다.
...
[릴리스 노트](https://helpx.adobe.com/experience-manager/6-3/release-notes/desktop-app-release-notes.html)
...
* [AEM Assets Brand Portal의 새로운 기능](https://helpx.adobe.com/experience-manager/brand-portal/using/whats-new.html)
* [릴리스 노트](https://helpx.adobe.com/experience-manager/brand-portal/release-notes/brand-portal-release-notes.html)
...
[Livefyre Feature Pack 2.0.6 릴리스](https://helpx.adobe.com/experience-manager/6-4/sites/administering/using/livefyre.html) 및 [Feature Packs](https://helpx.adobe.com/experience-manager/6-4/release-notes/livefyre-feature-pack-206.html) 페이지에 대한 새 워크플로와 [Livefyre를 통합](https://helpx.adobe.com/experience-manager/6-4/release-notes/feature-packs-release-notes.html) 업데이트합니다.

**Cloud Manager**

Cloud Manager 2018.6.0은 2018년 8월 9일부터 사용 가능합니다. 이 업데이트는 배포 중 디스패처 무효화, 추가 알림 및 사용성 개선에 대한 지원을 추가합니다.

자세한 정보는 [릴리스 정보 및 문서 업데이트](https://helpx.adobe.com/experience-manager/cloud-manager/using/release-notes-2018-6-0.html)를 검토하십시오.

**단일 페이지 애플리케이션**

AEM SPA 편집기의 [단일 페이지 애플리케이션(SPAs)](https://helpx.adobe.com/experience-manager/6-4/sites/developing/user-guide.html?topic=/experience-manager/6-4/sites/developing/morehelp/spa.ug.js) 개발을 위한 문서가 업데이트 및 확장됩니다.

**리치 텍스트 편집기**

[리치 텍스트 편집기 구성](https://helpx.adobe.com/experience-manager/6-4/sites/administering/using/rich-text-editor.html) 항목을 효율적으로 구조화하고, 누락된 정보를 추가하고, 워크플로 일러스트레이션을 추가하고, 긴 형식의 문서를 분할하여 개선합니다.

**AEM Assets**

oAuth 통합을 구성하려면 Adobe I/O 게이트웨이 서비스를 [https://legacy-oauth.cloud.adobe.io](https://legacy-oauth.cloud.adobe.io/)에서 사용할 수 있습니다.

Adobe는 [https://marketing.adobe.com/](https://marketing.adobe.com/)의 서비스를 중단했습니다.

자세한 내용은 [Marketing Cloud 및 Creative Cloud에서 AEM Assets 통합 구성](https://helpx.adobe.com/experience-manager/6-4/sites/administering/using/configure-assets-cc-integration.html) 및 [Brand Portal에서 AEM Assets 통합 구성](https://helpx.adobe.com/experience-manager/6-4/assets/using/brand-portal-configuring-integration.html)을 참조하십시오.
...
90개 이상의 세션에 무제한 액세스할 수 있는 On-Demand Pass를 구입할 수 있습니다. [여기](https://immerse18.adobe-devs.adobeevents.com/register/)로 이동하여 할인 코드 He7B52를 사용하십시오.
...
동료를 만나보십시오! 로컬 사용자 그룹과의 만남이 계속 증가하고 있습니다. [여기](https://www.meetup.com/pro/aem/)에서 귀하의 분야에 해당하는 동료가 있는지 확인하십시오. 로컬 그룹이 표시되지 않지만 관심이 있으십니까? [aemusers@adobe.com으로 이메일을 보내주시기 바랍니다.](mailto:aemusers@adobe.com)
