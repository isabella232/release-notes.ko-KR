...
| 중국에서 RDC 변경 사항 | 중국의 Adobe 지역 데이터 수집을 사용하여 중국 내에 있는 고객이 전 세계 다른 위치가 아닌 중국 내의 데이터 수집 센터로 직접 데이터를 보낼 수 있습니다. 이렇게 하면 중국 외부에 있는 데이터 수집 센터로 데이터를 전송하는 것에 비해 페이지 로드 시간 및 데이터 정확도가 향상됩니다. [자세히](https://marketing.adobe.com/resources/help/en_US/reference/rdc-china.html) |
...
| 새로운 라이브 스트림 변수 | * **carrier**: 모바일 장치에 대한 무선 제공 업체입니다. <br/>* **tnt:** Target에 사용된 Test-and-Target 필드입니다. _actionId_ 및 _actionName_에 값이 없습니다.<br/>* **Tntaction:** Target에 사용된 향상된 테스트 및 타겟 필드입니다. tnt 필드와 달리 _actionId_ 및 _actionName_에는 값이 설정되어 있습니다. |
...
### Chromecast, Roku, Apple TV 및 Launch Web Extension용 Media Analytics SDK 2.2 {#aa-va}
...
| Ad Break 유지 관리하기 | 플레이어가 ad break를 입력하고 특정 기간 동안 통화가 발신되지 않은 경우 플레이어가 ad break 상태여도 SDK 논리가 컨텐츠 재생으로 기본 설정되는 시나리오에서 광고 추적이 향상되었습니다. 프리롤 광고 동안, 이를 통해 컨텐츠가 시작된 후 ad break가 완료될 수 있습니다. Media 2.2 SDK에서는 광고 사이의 대기 시간이 긴 경우 ad break 내의 광고 사이에 버퍼 호출이 전송될 수 있습니다. |
...
* **분석 작업 공간:** 맵 시각화에 데이터가 표시되지 않던 문제를 수정했습니다. (AN-170626)
* Analytics의 (마이그레이션된) 사용자 성/이름이 Experience Cloud의 사용자 Adobe ID에 연결된 이름과 다른 문제가 해결되었습니다. Analytics 이름을 Experience Cloud에서 업데이트하려면 **[! Uicontrol 관리** &gt; **!UICONTROL 사용자 관리 (기존)**&gt; 사용자**!UICONTROL 편집을** 클릭하고 사용자 ID를 클릭합니다. 그런 다음 **!UICONTROL Experience Cloud에서 업데이트**를 클릭합니다. (AN-172825)
...
* 이제 워크플로우 내에서 또는 동일한 캠페인 인스턴스의 다른 워크플로우 내에서 활동을 복사하여 붙여넣을 수 있습니다. 이렇게 하면 전체 워크플로우 또는 특정 활동을 쉽게 복제하고 처음 정의된 설정을 유지할 수 있습니다. 자세한 내용은 [상세 문서](https://helpx.adobe.com/campaign/standard/automating/using/workflow-interface.html#duplicating-workflow-activities)를 참조하십시오. (CAMP-20014)
...
Adobe Livefyre의 새로운 기능, 수정 내용 및 업데이트. Livefyre에서 소셜 큐레이션 기능과 온사이트 참여 응용 프로그램을 포함하여 사용자가 생성한 전체 컨텐츠 서비스를 제공합니다.

>참고:
>Livefyre는 Adobe 보안 표준을 준수하면서 HTTP 및 TLS 1.1 및 이전 버전에 대한 지원을 중단할 예정입니다. &gt;지원 중단 날짜에 대해서는 이후 릴리스 노트를 참조하십시오.
...
| 기능 | 설명 |
| -----------| ---------- |  
| 포트폴리오 및 검색 캠페인 | 이제 입찰 유닛 세부 정보에서 모든 포트폴리오에 대한 내역 및 예측 탭에 컴퓨터와 태블릿에 대한 개별 탭이 포함됩니다. 이전에는 최적화 기능의 v 7를 사용하는 포트폴리오에서만 별도의 탭을 사용할 수 있었습니다. <br/> v 7를 사용하는 포트폴리오의 경우 컴퓨터 및 태블릿 탭의 모든 데이터는 별개입니다.<br/>그러나 최적화 기능의 v 6를 사용하는 포트폴리오의 경우 컴퓨터와 태블릿 탭에는 컴퓨터 및 태블릿에 대해 이전에 표시되었던 컴퓨터 및 태블릿의 총계가 모두 표시됩니다.<br/>**참고:** 최적화 방법이 v 6에서 v 7로 바뀔 때마다 그 반대의 경우도 마찬가지입니다. |
| 캠페인 검색 | 이제 여러 검색 엔진 계정을 한 번에 활성화 및 비활성화할 수 있습니다.<br/>(Bing 광고 계정) 기존 Bing 광고 사용자 지정 대상이 이제 대상 &gt; 라이브러리 보기에 포함되며 Bing 광고 광고 그룹의 타겟으로 사용할 수 있습니다. 각 대상 대상에 대한 입찰 수정자를 선택적으로 설정할 수 있습니다. |
