...
제품 설명서의 경우 [Experience Cloud ID 서비스](https://docs.adobe.com/content/help/en/id-service/using/home.html)를 참조하십시오.
...
| 기능 | 설명 |
| -----------| ---------- |  
| 지역 데이터 수집 변경 사항 | 데이터 수집 성능을 향상시키기 위해 나머지 모든 고객은 가능한 한 최종 사용자에게 가깝게 수집되도록 RDC(지역 데이터 수집)로 전환되었습니다. 이렇게 하면 사이트/앱 성능이 향상되고 최종 사용자 경험을 최적화하기 위해 데이터를 가능한 한 빨리 수집할 수 있습니다.<br/> 디지털 속성의 데이터가 지역적으로 수집되면 보안 연결을 통해 데이터 처리 센터로 전달됩니다. 이 데이터 처리 센터에서 처리되고 Adobe Experience Cloud의 제품에서 사용할 수 있게 됩니다. RDC는 10년 이상 새 구현을 위한 기본 방법이었습니다. [자세히](https://marketing.adobe.com/resources/help/ko_KR/reference/regional-data-collection.html) |
| 중국 내에서 RDC로 변경 | 중국 본토의 Adobe의 지역 데이터 수집을 사용하면 중국 내 고객은 전 세계 다른 위치가 아니라 중국 내의 데이터 수집 센터로 데이터를 바로 전송할 수 있습니다. 이렇게 하면 중국 외부의 데이터 수집 센터로 데이터를 전송하는 것보다 페이지 로드 시간과 데이터 정확도가 향상됩니다. [자세히](https://marketing.adobe.com/resources/help/ko_KR/reference/rdc-china.html) |
| 중국 지역 가속 | 이제 Analytics 보고서 세트 설정에서 중국 가속 옵션을 사용할 수 있습니다. 이 옵션은 중국 본토 사용자에 대한 Adobe Analytics 보고 사용자 환경을 개선합니다. 각 Adobe Analytics 사용자는 이 옵션을 활성화하거나 비활성화할 수 있습니다. 성능에 부정적인 영향을 미치기 때문에 중국 본토 이외의 사용자를 위해 이 옵션을 선택하지 마십시오. [자세히](https://marketing.adobe.com/resources/help/ko_KR/sc/user/report_settings.html) |
...
| Ad Break 유지 관리하기 | 플레이어가 ad break를 입력하고 특정 기간 동안 통화가 발신되지 않은 경우 플레이어가 ad break 상태여도 SDK 논리가 콘텐츠 재생으로 기본 설정되는 시나리오에서 광고 추적이 향상되었습니다. 프리롤 광고 동안, 이를 통해 콘텐츠가 시작된 후 ad break가 완료될 수 있습니다. Media 2.2 SDK에서 광고 사이에 긴 지연이 있을 때 광고 중단 내의 광고 간에 버퍼 호출이 전송됩니다. |
| SDK 이름 변경 | 2.2 릴리스부터 VHL(비디오 하트비트 라이브러리) SDK가 미디어 SDK로 이름이 변경되었습니다. Media 2.2 SDK는 VHL 2.X SDK 시리즈와 완전히 역호환됩니다. 이름 변경은 기능 나누기를 나타내지 않고 이름 지정 규칙의 변경을 의미합니다. |
...
* Analytics의 (마이그레이션된) 사용자 성/이름이 Experience Cloud의 사용자 Adobe ID에 연결된 이름과 다른 문제가 해결되었습니다. Analytics 이름을 Experience Cloud에 있는 이름으로 업데이트하려면 **[!UICONTROL 관리]** > **[!UICONTROL 사용자 관리(기존)]** > **[!UICONTROL 사용자 편집]**&#x200B;으로 이동하고 사용자 ID를 클릭합니다. 그런 다음 **[!UICONTROL Experience Cloud]**&#x200B;에서 업데이트를 클릭합니다. (AN-172825)
* UI에서 사용자가 실제로 권한을 부여하지 않았을 때 권한이 있음을 암시하는 웹 서비스 권한 표시 문제를 해결했습니다. (AN-168076)
...
| [글로벌 데이터 소스](https://marketing.adobe.com/resources/help/en_US/aam/global-data-sources.html) | 글로벌 데이터 소스는 모든 Audience Manager 고객이 액세스할 수 있으며 Apple, Samsung, Microsoft, Roku 및 Android 디바이스 제조업체와 같은 디바이스 제조업체에서 생성된 디바이스 광고 ID를 포함합니다. 이러한 ID는 제조업체에서 광고 목적으로 사용할 수 있습니다. 글로벌 데이터 소스를 사용하여 장치 ID를 동기화하고 그러한 매핑에서 데이터를 가져오거나 내보낼 수 있습니다.<br/>Audience Manager는 해당 형식을 기반으로 하여 고객이 가져온 DAID(장치 광고 ID)를 확인하여, 장치 제조 업체에 설명한 표준 형식과 일치하는지 확인합니다. |
| [ID 매핑 제한](https://marketing.adobe.com/resources/help/en_US/aam/usage-limits.html) | 장치 ID에 연결된 장치 간 ID 매핑 수를 제한하여 세그먼트의 세그멘테이션, 보고 및 내보내기를 개선했습니다. 새로 적용된 제한은 잘못된 통합 및 부정확한 데이터 세트가 섭취 및 다운스트림 처리에 미치는 영향을 완화하는 데 도움이 됩니다. 새 한계는 다음과 같습니다.<ul><li> 크로스 디바이스 ID당 100개의 디바이스 ID</li><li>디바이스 ID당 10개의 크로스 디바이스 ID</li><li> 1000개 장치 ID를 1개 장치 ID</li></ul> |

**수정 사항**

* 규칙 기반 트레이트에 맞게 온보드 트레이트에 대한 세그멘테이션 논리를 개선했습니다. (AAM-40433)
* 이제 [특성 요약 페이지](https://experiencecloud.adobe.com/resources/help/en_US/aam/c_tb_trait_summary.html)에 트레이트 유형이 표시됩니다. (AAM-25784)
...
사용자 참여를 보다 직관적으로 측정할 수 있도록 푸시 채널 보고에 몇 가지 향상된 기능이 추가되었습니다. 이번 릴리스를 통해 푸시 채널 지표 목록을 3개의 다른 지표로 확장합니다.
...
이 릴리스에는 Adobe Experience Platform Launch 및 Mobile SDK에서 Adobe Campaign Standard용 GA(Android) 버전 및 iOS 확장 기능과 Adobe Campaign의 통합이 포함되어 있습니다. 이러한 익스텐션은 푸시 메시지, 인앱 메시지 및 모바일 앱 프로파일 업데이트를 지원합니다.
...
이 릴리스에는 Campaign의 인앱 채널의 GA 버전이 포함됩니다. 기능 측면에서 볼 때 베타 릴리스에 추가된 가장 주목할 만한 기능은 인앱 채널에 대한 동적 보고서와 Mobile SDK와 MCIAS(SDK에 인앱 규칙을 제공하는 Marketing Cloud 인앱 메시징 서비스) 간의 보안 핸드셰이크입니다. 보안 핸드셰이크는 사용자의 PII 데이터가 악의적인 수중에 빠지지 않도록 보장하고 사용자가 로그아웃할 때마다 메시지 캐시를 지우면서 공유 장치에서 사용자의 개인 정보를 유지할 수 있도록 해줍니다.
...
* 이제 **Load 파일** 활동을 사용할 때 거부된 레코드가 포함된 파일의 이름에 타임스탬프를 추가할 수 있습니다. 자세한 내용은 [상세 문서](https://helpx.adobe.com/kr/campaign/standard/automating/using/load-file.html#configuration)를 참조하십시오.
* **이제 활동** 에서 데이터를  **** 검색하지 않는 경우 아웃바운드 전환을 활성화할 수 있습니다.
...
Cloud Manager 2019.2.0 릴리스에는 시스템 모니터링이 추가됩니다. 이 새로운 기능을 통해 고객은 Adobe Managed Services 환경의 상태를 시스템 수준에서 볼 수 있습니다.
...
Brand Portal 6.4.4 릴리스에는 사용자에 대한 자산 검색 환경을 개선할 수 있는 개선 사항이 포함됩니다. 부분 구문 검색 및 와일드카드 검색 - 속성 조건자와 상위 고객 요청에 중점을 둡니다.

자세한 내용은 [AEM Assets 브랜드 포털의 새로운 기능](https://helpx.adobe.com/kr/experience-manager/brand-portal/using/whats-new.html) 및 [릴리스 노트](https://helpx.adobe.com/kr/experience-manager/brand-portal/release-notes/brand-portal-release-notes.html)를 참조하십시오.
...
이 컨텐츠는 프레젠테이션의 일부 개선 사항과 내용이 동일합니다. 설명서 제안을 제공하는 새로운 기능을 통해 더 흥미롭게 느껴보십시오. 모든 페이지에서 **[!UICONTROL 이 페이지 편집]** 링크 및 **[!UICONTROL 문제 기록]** 링크를 사용하여 GitHub로 가져오기 요청 또는 문제를 해결할 수 있습니다.
...
* Instagram Business Account 토큰을 새로 고치면 &quot;중복된 계정&quot; 오류가 발생하는 문제를 해결했습니다.
* 사용자 정의 스크립트를 사용하여 앱을 업데이트할 때 오류가 발생하여 중복 앱이 만들어지는 문제를 수정했습니다.
