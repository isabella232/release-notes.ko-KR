...
| Ad Break 유지 관리하기 | 플레이어가 ad break를 입력하고 특정 기간 동안 통화가 발신되지 않은 경우 플레이어가 ad break 상태여도 SDK 논리가 콘텐츠 재생으로 기본 설정되는 시나리오에서 광고 추적이 향상되었습니다. 프리롤 광고 동안, 이를 통해 콘텐츠가 시작된 후 ad break가 완료될 수 있습니다. Media 2.2 SDK에서 광고 사이에 긴 지연이 있을 때 광고 중단 내의 광고 사이에 버퍼 호출이 전송됩니다. |
