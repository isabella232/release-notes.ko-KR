...
* 릴리스 버전 4.1.
* 새로운 API 변경 사항에 따라 publishDestinations를 업데이트합니다.
...
| 기능 | 설명 |
| -----------| ---------- |  
| 지역 데이터 수집 변경 사항 | 데이터 수집 성능을 향상시키기 위해 나머지 모든 고객은 가능한 한 최종 사용자에게 가깝게 수집되도록 RDC(지역 데이터 수집)로 전환되었습니다. 이렇게 하면 사이트/앱 성능이 향상되고 최종 사용자 경험을 최적화하기 위해 가능한 한 빨리 데이터를 수집할 수 있습니다.<br/> 디지털 속성의 데이터가 지역적으로 수집되면 보안 연결을 통해 데이터 처리 센터로 전달됩니다. 이 데이터 처리 센터에서 처리되고 Adobe Experience Cloud의 제품에서 사용할 수 있게 됩니다. RDC는 10년 이상 새 구현을 위한 기본 방법이었습니다. [자세히](https://marketing.adobe.com/resources/help/ko_KR/reference/regional-data-collection.html) |
| 중국 내에서 RDC로 변경 | 중국 본토의 Adobe 지역 데이터 수집을 사용하면 중국 내 고객은 전 세계 다른 위치가 아니라 중국 내의 데이터 수집 센터로 데이터를 바로 전송할 수 있습니다. 이렇게 하면 중국 외부의 데이터 수집 센터로 데이터를 전송하는 것과 비교하여 페이지 로드 시간과 데이터 정확도가 향상됩니다. [자세히](https://marketing.adobe.com/resources/help/ko_KR/reference/rdc-china.html) |
| 중국 지역 가속 | 이제 Analytics 보고서 세트 설정에서 중국 가속 옵션을 사용할 수 있습니다. 이 옵션은 중국 본토에 있는 사용자에 대한 Adobe Analytics 보고 사용자 환경을 개선합니다. 각 Adobe Analytics 사용자는 이 옵션을 활성화하거나 비활성화할 수 있습니다. 성능에 부정적인 영향을 미치기 때문에 중국 본토 이외의 사용자를 위해 이 옵션을 선택하지 마십시오. [자세히](https://marketing.adobe.com/resources/help/ko_KR/sc/user/report_settings.html) |
...
| API 2.0: JWT 인증 지원 | 이제 Adobe Analytics 2.0 API가 JWT 인증을 지원합니다. JWT 인증에 대한 자세한 내용은 [https://github.com/AdobeDocs/analytics-2.0-apis/blob/master/jwt.md](https://github.com/AdobeDocs/analytics-2.0-apis/blob/master/jwt.md) |
...
| Ad Break 유지 관리하기 | 플레이어가 ad break를 입력하고 특정 기간 동안 통화가 발신되지 않은 경우 플레이어가 ad break 상태여도 SDK 논리가 콘텐츠 재생으로 기본 설정되는 시나리오에서 광고 추적이 향상되었습니다. 프리롤 광고 동안, 이를 통해 콘텐츠가 시작된 후 ad break가 완료될 수 있습니다. Media 2.2 SDK에서 광고 사이에 긴 지연이 있는 경우 광고 중단 내의 광고 사이에 버퍼 호출이 전송됩니다. |
...
* Analytics의 (마이그레이션된) 사용자 성/이름이 Experience Cloud의 사용자 Adobe ID에 연결된 이름과 다른 문제가 해결되었습니다. To update the Analytics name with the one in Experience Cloud, go to **[!UICONTROL Admin]** > **[!UICONTROL User Management (Legacy)]** > **[!UICONTROL Edit Users]** and click the User ID. 그런 다음 Experience Cloud **[!UICONTROL 에서 업데이트를 클릭합니다]**. (AN-172825)
* UI에서 사용자가 실제로 권한이 없을 때 권한이 있음을 암시하는 웹 서비스 권한 표시 문제를 수정했습니다. (AN-168076)
...
* **[!UICONTROL 제외할 트레이트 선택]** 페이지에서 알고리즘 모델의 [모두 선택](https://experiencecloud.adobe.com/resources/help/en_US/aam/trait-exclusion-algo-models.html) 확인란이 누락된 버그가 수정되었습니다. (AAM-42462)
...
| 기능 | 설명 |
| -----------| ---------- |  
| 시각적 경험 작성기 | 시각적 경험 작성기(VEC)에는 다음 개선 사항이 포함되어 있어 작업을 보다 빠르고 효율적으로 수행할 수 있습니다.<ul><li>이제 VEC에서 웹 사이트 로딩을 취소하여 활동 편집 차단을 해제할 수 있습니다. 이 개선 사항은 예를 들어 활동에 대해 약간 편집하거나, 설정을 검토하거나, 사용자 지정 코드를 추가하려는 경우 사이트가 로드될 때까지 기다리지 않으려는 경우 유용합니다. 사이트를 로드하기 전에 편집할 수 없는 작업은 Target UI에서 비활성화됩니다. (TGT-31288, TGT-31611 및 TGT-32602)</li><li>VEC에 DOM 트리가 표시되므로 경험을 만들거나 편집하는 동안 적절한 요소를 쉽게 선택할 수 있습니다. (TGT-13422)</li></ul> |

**개선 사항, 수정 사항, 변경 사항**

이제 보고서를 검토하는 동안 세션이 만료될 때 다시 인증하라는 메시지가 표시됩니다. 다시 로그인하면 보고서로 다시 돌아갑니다. (TGT-32723)
...
푸시 채널 보고에 사용자 참여를 보다 직관적으로 측정할 수 있도록 향상된 몇 가지 기능이 추가되었습니다. 이번 릴리스에서는 푸시 채널 지표 목록을 세 개의 다른 지표로 확장하고 있습니다.

* 노출 횟수
* 클릭 수
* 열기(앱 열기)

이러한 기능은 푸시 알림과 사용자의 상호 작용을 보다 효과적으로 측정하고 분석하는 데 도움이 됩니다. 또한 이러한 지표의 정의 및 구현을 표준화하고 있습니다. 푸시 알림 내장 보고서도 일반적으로 사용되는 시각화 및 지표와 함께 향상되었습니다.

자세한 내용은 푸시 [알림 보고서를 참조하십시오](https://helpx.adobe.com/kr/campaign/standard/reporting/using/push-notification-report.html).

### [!DNL Launch] 모바일 앱용 통합

이 릴리스에는 Adobe Experience Platform Launch 및 모바일 SDK의 Adobe Campaign Standard용 GA(일반 사용 가능) 버전과 Adobe Campaign의 통합이 포함되어 있습니다. 이러한 익스텐션은 푸시 메시지, 인앱 메시지 및 모바일 앱 프로필 업데이트를 지원합니다.

자세한 내용은 Adobe Experience Platform SDK [사용을 참조하십시오](https://helpx.adobe.com/kr/campaign/standard/administration/using/configuring-a-mobile-application.html#main-pars_header_49).

### 모바일 인앱 메시지

이 릴리스에는 Campaign의 인앱 채널의 GA 버전이 포함됩니다. 기능 측면에서 베타 릴리스의 가장 주목할 만한 추가 사항은 인앱 채널에 대한 동적 보고서와 Mobile SDK와 MCIAS(In-App 규칙을 SDK에 제공하는 Marketing Cloud 인앱 메시징 서비스) 간의 안전한 핸드셰이크입니다. 보안 핸드셰이크는 사용자의 PII 데이터가 악의적인 수중에 노출되지 않도록 하며, 사용자가 로그아웃할 때마다 메시지 캐시를 지우면서 공유 장치에서 사용자의 개인 정보를 유지할 수 있도록 해줍니다.

자세한 내용은 [인앱 메시징](https://helpx.adobe.com/kr/campaign/standard/channels/using/about-in-app-messaging.html) 정보 및 전용 [인앱 자습서를 참조하십시오](https://helpx.adobe.com/kr/campaign/kt/acs/using/acs-in-app-message-tutorial.html).

### 향상된 워크플로우

다음과 같은 워크플로우 기능이 추가되었습니다.

* 이제 동일한 Campaign 인스턴스의 워크플로우 또는 다른 워크플로우 내에서 활동을 복사하여 붙여넣을 수 있습니다. 이 방법으로 전체 워크플로우 또는 특정 활동을 쉽게 복제하고 초기에 정의된 설정을 유지할 수 있습니다. 자세한 내용은 [상세 문서](https://helpx.adobe.com/kr/campaign/standard/automating/using/workflow-interface.html#duplicating-workflow-activities)를 참조하십시오. (CAMP-20014)
* 이제 파일 **로드** 활동을 사용할 때 거부된 레코드가 포함된 파일의 이름에 타임스탬프를 추가할 수 있습니다. 자세한 내용은 [상세 문서](https://helpx.adobe.com/kr/campaign/standard/automating/using/load-file.html#configuration)를 참조하십시오.
* **이제 쿼리** 및 **세그멘테이션** 활동을 통해 활동이 데이터를 검색하지 않는 경우 아웃바운드 전환을 활성화할 수 있습니다.
...
Cloud Manager 2019.2.0 릴리스에는 시스템 모니터링이 추가되었습니다. 이 새로운 기능을 통해 고객은 Adobe Managed Services 환경의 상태를 시스템 수준에서 볼 수 있습니다.

관련 릴리스 [노트를 참조하십시오](https://docs.adobe.com/content/help/ko-KR/experience-manager-cloud-manager/using/release-notes/release-notes-current.html).
...
브랜드 포털 6.4.4 릴리스에는 사용자에 대한 자산 검색 환경을 개선하기 위한 개선 사항이 포함됩니다. 부분 구문 검색 및 와일드카드 검색 기능 속성 설명 및 상위 고객 요청에 중점을 둡니다.

자세한 내용은 AEM Assets 브랜드 포털 [의 새로운 기능](https://helpx.adobe.com/kr/experience-manager/brand-portal/using/whats-new.html) 및 [릴리스 노트를 참조하십시오](https://helpx.adobe.com/kr/experience-manager/brand-portal/release-notes/brand-portal-release-notes.html).
...
프레젠테이션의 일부 개선 사항과 내용이 동일합니다. 더욱 흥미로운 것은 설명서 제안을 제공하는 새로운 기능입니다. 모든 페이지에서 **[!UICONTROL 이 페이지 편집]** 링크 및 **[!UICONTROL 문제 기록]** 링크를 사용하여 GitHub로 가져오기 요청 또는 문제를 해결할 수 있습니다.
...
* Instagram Business Account 토큰을 새로 고치면 &quot;중복된 계정&quot; 오류가 발생하는 문제를 수정했습니다.
* 사용자 정의 스크립트를 사용하여 앱을 업데이트하면 중복된 앱이 만들어지는 문제가 해결되었습니다.
...
See [Livefyre release notes](https://docs.adobe.com/content/help/ko-KR/livefyre/using/release-notes/c-rn.html) for the latest information.
