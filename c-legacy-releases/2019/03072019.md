...
| 중국 내에서 RDC로 변경 | 중국 본토의 Adobe 지역 데이터 수집을 사용하면 중국 내 고객은 전 세계 다른 지역이 아닌 중국 내의 데이터 수집 센터로 데이터를 직접 전송할 수 있습니다. 이렇게 하면 중국 외부의 데이터 수집 센터로 데이터를 전송하는 것과 비교하여 페이지 로드 시간과 데이터 정확도가 향상됩니다. [자세히](https://marketing.adobe.com/resources/help/ko_KR/reference/rdc-china.html) |
| 중국 지역 가속 | 이제 Analytics 보고서 세트 설정에서 중국 가속 옵션을 사용할 수 있습니다. 이 옵션은 중국 본토에서 사용자를 위한 Adobe Analytics 보고 사용자 환경을 개선합니다. 각 Adobe Analytics 사용자는 이 옵션을 활성화하거나 비활성화할 수 있습니다. 성능에 부정적인 영향을 미치기 때문에 중국 본토 이외의 사용자를 위해 이 옵션을 선택하지 마십시오. [자세히](https://marketing.adobe.com/resources/help/ko_KR/sc/user/report_settings.html) |
...
* Analytics의 (마이그레이션된) 사용자 성/이름이 Experience Cloud의 사용자 Adobe ID에 연결된 이름과 다른 문제가 해결되었습니다. Analytics 이름을 Experience Cloud의 이름으로 업데이트하려면 **[!UICONTROL 관리자** > **!UICONTROL 사용자 관리(레거시)**>**!UICONTROL 사용자 편집**&#x200B;으로 이동하고 사용자 ID를 클릭하십시오. 그런 다음 Experience Cloud **!UICONTROL 에서 업데이트를 클릭합니다&#x200B;**. (AN-172825)
...
이 릴리스에는 Adobe Experience Platform Launch 및 Mobile SDK에서 Android 및 Adobe Campaign Standard용 iOS 확장 프로그램의 일반 사용 가능(GA) 버전과 Adobe Campaign의 통합이 포함되어 있습니다. 이러한 익스텐션은 푸시 메시지, 인앱 메시지 및 모바일 앱 프로필 업데이트를 지원합니다.
...
이 릴리스에는 Campaign의 인앱 채널의 GA 버전이 포함됩니다. 기능 측면에서 베타 릴리스의 가장 주목할 만한 추가 사항은 인앱 채널에 대한 동적 보고서와 SDK와 MCIAS(SDK에 In-App 규칙을 제공하는 Marketing Cloud 인앱 메시징 서비스) 간의 안전한 핸드셰이크입니다. 보안 핸드셰이크는 사용자의 PII 데이터가 악의적인 수중에 노출되지 않도록 하며, 사용자가 로그아웃할 때마다 메시지 캐시를 지우면서 공유 장치에서 사용자의 개인 정보를 유지할 수 있도록 해줍니다.
