...
이 기능을 통해 기업이 청취자의 도달 범위를 측정하고, 컨텐츠의 인기를 측정하며, 사용자의 행동과 유사 모델링을 기반으로 사용자의 참여를 유지합니다. 업계 최고의 SDK를 사용하여 10초 오디오 컨텐츠 및 1초 스트리밍 광고를 측정하여 경험의 품질(중지, 오류, 시작 시간)과 컨텐츠 및 광고 KPI를 평가할 수 있습니다.

**Ad Break 유지 관리하기**

플레이어가 ad break를 입력하고 특정 기간 동안 통화가 발신되지 않은 경우 플레이어가 ad break 상태여도 SDK 논리가 컨텐츠 재생으로 기본 설정되는 시나리오에서 광고 추적이 향상되었습니다. 프리롤 광고 동안, 이를 통해 컨텐츠가 시작된 후 ad break가 완료될 수 있습니다. Media 2.2 SDK에서는 광고 사이의 대기 시간이 긴 경우 ad break 내의 광고 사이에 버퍼 호출이 전송될 수 있습니다.
...
* **보고 및 분석:** 사용자가 보고서를 다른 사용자와 공유하지 못했던 문제를 수정했습니다. (AN-172301)
* **관리:** 이전 관리자 사용자 관리의 편집 기능 문제를 수정했습니다. 사용자를 선택하고 편집을 클릭하면 해당 사용자에 대한 데이터가 표시되지 않았습니다. (AN-170705)
* Analysis Workspace에서 특정 오류에 대한 일부 재시도 논리가 추가되었습니다. (AN-172801)

### JavaScript 용 appmeasurement {APPM}

JavaScript 2.11.0 - 2 월 11 일 출시
...
* 검색어가 백슬래시 문자로 끝날 때 트레이트, 세그먼트, 대상 및 모델에 대한 검색 필드가 표시되지 않는 문제를 해결했습니다.
* 피드 사용량 보고서의 미지급 섹션에 잘못된 날짜가 표시되는 문제가 수정되었습니다.

## Experience Manager {#aem}

Adobe Experience Manager의 새로운 기능, 수정 사항 및 업데이트 안정성, 보안 및 성능 향상을 위해 최신 패치를 배포하려는 경우 온-프레미스 배포를 사용하는 것이 좋습니다.
