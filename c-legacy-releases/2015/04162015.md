...
<table id="table_04854D12F4C241AE86F24C12C4F0FA28"> 
 ...
 <tbody> 
  <tr> 
   ...
   <td colname="col2"> <p>사용자 및 그룹 관리 기능이 Enterprise Dashboard로 이동되었습니다. 새로운 이동 경로는 다음과 같습니다. </p> <p> <span class="uicontrol"> Marketing Cloud</span> &gt; <span class="uicontrol">도구</span> &gt; <span class="uicontrol">Enterprise Dashboard 시작 </span></p> <p> 또한 Enterprise ID 및 Federated ID에 대한 지원을 이용할 수 있습니다. 동일한 엔터프라이즈 배포에서 Enterprise ID, Federated ID 및 Adobe ID를 사용할 수 있습니다. 예를 들어, 다른 Adobe 제품 및 서비스를 사용하는 사용자의 경우 Adobe ID를 사용하십시오. 계정을 엄격히 관리하기를 바라는 사용자의 경우에는 Enterprise ID나 Federated ID를 사용하십시오. </p> <p><a href="https://docs.adobe.com/content/help/ko-KR/core-services/interface/manage-users-and-products/admin-getting-started.html" format="https" scope="external">사용자 및 그룹 - ID 관리</a>를 참조하십시오 . </p> </td> 
  </tr> 
 </tbody> 
</table>
...
* 이제 [!UICONTROL 피드백 제공] 도구를 사용하여 *`Bug`* 유형의 문제를 제출하면 이러한 문제가 고객 지원 팀으로 바로 제출되어 티켓이 생성됩니다. 그 후에 곧 고객 지원 팀에서 연락이 갑니다. 그 후에 고객 지원 센터에서 연락을 드릴 것입니다.
* 이제 인앱 메시지를 만들 때 또는 을 사용하여 메시지를 타깃팅할 수 *`Lifetime Value`* 있습니다 *`Locale`*.
 ...
 <tbody> 
  <tr> 
   <td colname="col1"> 데이터 웨어하우스 <p> 
     <!--AN-100995--></p> </td> 
   <td colname="col2"> <p>히트당 최대 행 수를 1000개에서 100개로 감소 </p> <p>데이터 웨어하우스 요청이 여러 개의 다중 값 차원과 함께 제출되면(예: 다양한 모바일 보고서), 행이 하나의 히트로부터 기하급수적으로 생성될 수 있습니다. 이제 단일 히트에서 출력할 수 있는 행 수를 100개(이전에는 1,000개)로 제한하고 있습니다. </p> </td> 
  </tr> 
 </tbody> 
</table>
...
* 대시보드를 수동으로 재설정해야 하는 비교 날짜 범위 설정 문제를 해결했습니다. 이 수정 사항은 새 책갈피에만 작동합니다.기존 버전을 열고 열어야 합니다 *`saved as`*.
...
* 데이터 웨어하우스에 세그멘테이션 지표로 [!DNL Analytics] 지원을 추가하여 보고서 &amp; *`campaign instances`* 및 데이터 웨어하우스 데이터 간에 불일치를 수정했습니다.
...
* 소셜 애플리케이션 기능은 Adobe Social 15.4.1.0(2015년 4월) 릴리스에서 공식적으로 더 이상 사용되지 않습니다. 여기에는 모든 애플리케이션 관련 지표, 데이터 및 API 메서드 제거가 포함됩니다. 질문이 있는 경우 소셜 계정 관리자에게 문의하십시오. For full details, see the [Adobe Social Applications - Deprecation Fact Sheet](https://helpx.adobe.com/social/kb/social-applications---deprecation-fact.html).
...
* 특히 오류가 발생한 경우 Facebook 표지 사진에 대한 업로드 기능이 향상되었습니다.
* 게시물을 복제할 때 하위 게시물과 마스터 게시물 간의 게시 시간이 일치하지 않던 문제를 수정했습니다.
* &quot;트랜잭션 격리 충돌이 감지됨&quot; 오류로 인해 게시물이 실패하던 문제를 수정했습니다.
* 링크 첨부 파일을 사용하여 LinkedIn 페이지에 게시된 게시물에 축소판 이미지가 표시되지 않는 문제를 해결했습니다.
* LinkedIn 페이지에 게시된 단축 URL이 오작동하는 문제를 해결했습니다.
