...
* 획득 캠페인 추적 만료가 모든 고객에 대해 방문에서 방문자로 변경되었습니다. 이 변경 사항은 첫 번째 방문만이 아니라 모든 방문에 대해 방문자에게 다운로드 후 활동에 대한 캠페인 속성을 용이하게 합니다.
* 다음 가이드를 재구성하고 개선했습니다.

   * [Adobe Mobile Services 사용 안내서](https://docs.adobe.com/content/help/ko-KR/mobile-services/using/home.html)
   * [Marketing Cloud 솔루션용 iOS SDK 4.x 관리 안내서](https://docs.adobe.com/content/help/ko-KR/mobile-services/ios/overview.html)
...
2015년 6월 18일 비디오 스트림 분석(하트비트) 릴리스에는 다음 변경 사항이 포함됩니다.
     ...
     <ul id="ul_F49FF181E93744F8BAD180B994073F20"> 
      ...
      <li id="li_8913320B99114DE881B5F4E8B3BD0D84"> <p> <b>비디오 경로</b> </p> 
       <ul id="ul_CCC6EA9299644D48935146DDD988DE36"> 
        ...
        <li id="li_5146C2C8E86F4DACB07A99E7939B360F"> <p>경로의 최대 깊이는 3입니다. </p> </li> 
       </ul> </li> 
     </ul> </p> </td> 
  </tr> 
  <tr> 
...
<table id="table_91D1FD20C4C1411292252364328677AF"> 
 ...
 <tbody> 
  <tr> 
   <td colname="col1"> 통합된 계산된 지표 </td> 
   <td colname="col2"> <p>이제 계산된 지표를 사용하면 세그먼트화된 지표, 더 진보된 수학 및 지표를 구성하고 공유할 수 있습니다. </p> <p>구현을 업데이트하지 않고 역사적으로 적용되는 새 지표를 만들고 지표를 공유 및 조정하여 불필요한 부분을 제거하고 기본 통계 분석을 수행할 수 있습니다. 세그먼트를 훨씬 더 쉽게 비교할 수 있습니다. </p> <p>Reports &amp; Analytics, 분석 작업 공간, 리포트 빌더 및 애드혹 분석의 어느 곳에서든 이 새로운 지표를 적용할 수 있습니다. </p> <p> <a href="https://marketing.adobe.com/resources/help/ko_KR/analytics/calcmetrics/" format="https" scope="external"> 자세히... </a> </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> Analysis Workspace - 자유 형식 분석 </td> 
   <td colname="col2"> <p>자유 형식 분석을 사용하면 차원, 지표 및 세그먼트를 드래그하여 놓아 원하는 보고서를 만들 수 있습니다. 관련 지표, 차원, 세그먼트, 타임라인 및 기타 분석 분류 값을 사용하여 쿼리를 작성할 수 있는 캔버스를 제공하여 특정 요구에 대한 데이터를 분류할 수 있습니다. </p> <p>최신 기능은 다음과 같습니다. </p> <p> 
     ...
     </ul> </p> <p> <a href="https://docs.adobe.com/content/help/ko-KR/analytics/analyze/analysis-workspace/home.html" format="https" scope="external">자유 형식 분석</a> 기능은 현재 주로 Analytics 커뮤니티 팀에서 지원되는 공개 베타에 있습니다. 자유 형식 분석에 대한 최신 정보를 알고 싶거나 질문하려면 <a href="https://help-forums.adobe.com/content/adobeforums/en/analytics-forum/freeform-analysis.html" format="http" scope="external">자유 형식 분석(베타) 포럼</a>을 참조하십시오 . </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> Data Warehouse SFTP 덮어쓰기 </td> 
   <td colname="col2"> 이제 SFTP가 전송에 대한 덮어쓰기를 지원합니다. SFTP 서버를 이름 변경을 지원하는 버전으로 업그레이드하는 것이 좋습니다. </td> 
  </tr> 
  <tr> 
   <td colname="col1"> 처리 규칙 제한 증가 </td> 
   <td colname="col2"> <p>허용된 처리 규칙 수가 100개에서 150개로 증가했습니다. 또한 처리 규칙 XML의 허용된 크기가 4MB(65KB에서)로 증가했습니다. 이 크기 변경으로 인해 XML 제한이 초과될 때 발생하는 오류가 방지됩니다. </p> </td> 
  </tr> 
  <tr> 
...
Analytics Premium은 [Data Workbench 6.4 릴리스에서 다음과 같은 새로운 추가 기능을 제공합니다](https://docs.adobe.com/content/help/ko-KR/data-workbench/using/release-notes/release-notes.html).
...
<table id="table_FE8C9A73676345CDA450CE84111A2B72"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td colname="col2"> 단계별 설치 마법사를 사용하여 Data Workbench(클라이언트)을 설정합니다. 워크벤치를 다운로드하고, 옵션을 설정하고, 워크벤치를 설치하고, 서버와 동기화합니다. </td> 
  </tr> 
  <tr> 
   <td colname="col1"> 향상된 워크스테이션 구성 환경 </td> 
   <td colname="col2"> 워크스테이션 설치 후, <b>서버</b> 작업 공간에 대한 연결 구성 작업 공간이 열리고 인증서 설치 및 Insight.cfg 파일에 서버 연결 정보 입력에 대한 추가 정보가 표시됩니다. 서버에 대한 연결 상태를 보고 프로필을 선택할 수도 있습니다. </td> 
  </tr> 
  <tr> 
   <td colname="col1"> 프레젠테이션 레이어 </td> 
   <td colname="col2"> 프레젠테이션 오버레이를 사용하여 시각화에 주석을 달고 명확하게 할 수 있습니다. 텍스트 설명선, 화살표, 이미지 및 색상 코딩을 추가하여 데이터를 강조 표시하고 명확하게 표시한 다음 다른 사용자와 공유할 수 있습니다. </td> 
  </tr> 
  <tr> 
   <td colname="col1"> 새 지표 Dimension 마법사 </td> 
   <td colname="col2"> 단계별 마법사를 사용하여 지표 차원 작성을 용이하게 합니다. </td> 
  </tr> 
  <tr> 
   <td colname="col1"> 그룹 구성원 액세스에 대한 사용자 관리 </td> 
   <td colname="col2"> 관리자는 워크스테이션 사용자에게 사용자 지정 그룹에 대한 액세스 제어를 관리할 수 있는 부분 기능을 제공할 수 있습니다. </td> 
  </tr> 
  <tr> 
   <td colname="col1"> 프로필 변경 방지 </td> 
   <td colname="col2"> 프로필, Dimension, 보고서, 작업 영역, 지표 또는 필터 관리자에서 작업하는 사용자가 프로필을 덮어쓰지 않도록 합니다. </td> 
  </tr> 
  <tr> 
   <td colname="col1"> 새 워크스테이션 사용자 인터페이스 업데이트 </td> 
  ...
  <tr> 
   ...
   <td colname="col2"> 이제 데이터 워크벤치에서는 필요에 따라 로깅을 구성하는 기능을 제공하는 확장된 로깅 프레임워크 "L4"를 사용합니다. 6.4 패키지와 함께 제공되는 기본 구현에서는 소프트웨어 처리에 대한 중요한 정보를 제공합니다. 로깅은 연결된 서버, 클라이언트 및 보고서 서버에 대한 추가 정보를 포함하여 서버 이벤트 문제를 해결하고 기본 문제를 분석하는 데 도움이 되는 추가 정보로 확장할 수 있습니다. <p>추가 L4 로깅 구현에 대한 추가 지원은 계정 관리자에게 문의하십시오. </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> ExportIntegration.exe 로깅 옵션을 위한 새 cfg 파일 </td> 
   <td colname="col2"> <p>새 httpLoggingEI.cfg 구성 파일(<span class="filepath">server\Admin\Export\httpLoggingEI.cfg</span>에 있음)을 사용하면 Export Integration 내보내기 동안 HTTP.log 파일에 대한 INFO 로깅을 중지할 수 있습니다. (CRS, TNT 및 MMP 내보내기 기능은 이미 개별 내보내기 로그 파일에서 자세한 로깅을 캡처합니다.) </p> <p>true 설정은 HTTP.log 파일에 대한 INFO 로깅(테스트 및 세부 보고용)을 시작하고, false 설정은 자세한 로깅을 중지합니다. false 설정의 경우 WARNING/ERROR 수준 메시지만 HTTP.log 파일로 전송됩니다. </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> 그래프 시각화에 대한 확대/축소 기능 </td> 
   <td colname="col2"> 값이 더 높은 차이를 보일 때 지표 레이블을 더 잘 보려면 확대/축소 기능을 사용하십시오. 이전에는 값과의 대비가 변경되면서 레이블이 사라졌습니다. 예를 들어 이전 값에 대해 더 높은 지표 회귀 값을 설정하는 경우. 이제 <b>&lt;Ctrl&gt;</b>을 클릭하고 마우스가 그래프를 가리키는 동안 마우스 휠을 움직여서 시각화 요소를 확대할 수 있습니다. </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td colname="col2"> 이제 이러한 실행 파일은 소프트웨어 다운로드가 변경되거나 손상되지 않았음을 보장하기 위해 디지털 서명을 받습니다. </td> 
  </tr> 
  <tr> 
   <td colname="col1"> 날짜 형식 옵션 </td> 
   <td colname="col2"> 표준 시간 <i>Dimension.cfg</i> 파일에서 로케일을 기준으로 날짜 형식을 변경할 수 있습니다. 기본 <i>MM/DD/YYYY</i> 형식을 <i>DD/MM/YYYY</i> 형식으로 변경하거나 다른 옵션을 선택하십시오. </td> 
  </tr> 
  <tr> 
   <td colname="col1"> 파일 시각화 단순화 </td> 
   <td colname="col2"> 보고 시 기본 프로필에 대한 파일 시각화(<span class="uicontrol">관리</span> &gt; <span class="uicontrol">파일</span>)에 더 큰 디렉토리(제거된 로그, 내보내기 및 조회)가 포함되지 않게 됩니다. 이렇게 하면 보고서 표시 속도가 증가합니다. 이제 더 큰 디렉토리에는 고유한 개별 보고서(관리 / 파일 내보내기, 파일 조회 및 로그 파일)가 있습니다. </td> 
  </tr> 
  <tr> 
   <td colname="col1"> DeviceAtlas 변환 개선 </td> 
   <td colname="col2"> 이제 DeviceAtlas.bundle 파일은 메모리 내 캐시를 사용하여 조회 성능을 크게 개선합니다. </td> 
  </tr> 
  <tr> 
   <td colname="col1"> 업데이트된 Chord 시각화 </td> 
   <td colname="col2"> Chord 시각화를 볼 때 섹션 위로 마우스를 가져가면 <b>가시성이</b> 개선되었습니다. </td> 
  </tr> 
  <tr> 
   <td colname="col1"> 차원을 파인더에서 세부 사항 테이블로 드래그 </td> 
...
* eVar의 값이 삭제되거나 처리 규칙을 사용하여 설정한 경우 eVar의 인스턴스가 증가되는 문제 *`blank`*&#x200B;를 해결했습니다.
...
* 동일한 작업 영역에서 여러 입력을 다시 실행할 때 성향 점수가 재설정되지 않았습니다. 이제 올바르게 재설정됩니다.
* 상관 관계 지표를 처음 열 때 사용할 수 있는 계산 가능한 차원이 없는 문제가 수정되었습니다.
* 필드가 누락되어 Target 세그먼트 내보내기에 *`mboxPC`* 실패했습니다. 이제 수정되었습니다.
* ID 요청 형식이 올바로 지정되었습니다. Using the *`mbox3rdpartyId`* identification instead of default PCIDs caused [!UICONTROL Adobe Target] to reject requests generated via the Target/Data Workbench integration (using the [!DNL  ExportIntegration.exe]). 이제 이 ID 요청의 형식이 올바로 지정되고 처리량이 성공적입니다.
...
* 방문 시간/사이트 시간 비즈니스 로직을 변경했습니다.
   ...
   * Use *`visit_start_time_gmt`* as the starting time of a visit. 이제 보고 날짜 범위 이전에 시작된 방문은 부정적인 영향을 받지 않습니다.
   * 0초 *`Group L: No duration`* 에 걸쳐 방문을 분류하는 새 분류 카테고리를 추가했습니다. 이제 이러한 방문은 더 이상 *`Group A: <1 minute`* 시간 버킷에 포함되지 않습니다.
   * 트랜잭션 데이터 소스 히트(hit_source=7)는 더 이상 방문 지속 시간이 있을 것으로 간주되지 않습니다.
   * 타임스탬프가 있는 타임스탬프가 시간 삼각주에 적용된 *`visit_start_time_gmt`* 절대값보다 더 빨리 지정되므로 항상 자동으로 분류되지 않습니다 *`Group A: <1 minute`*.
...
* 인증된 상태 지원. 방문자 ID와 함께 방문자 API는 여러 고객 ID와 다른 고객 ID의 범위를 구분하기 위해 동일한 방문자의 여러 고객 ID와 고객 유형 식별자를 수락합니다 *`authStates`*.
...
* Swagger 관련 API 설명서를 개선하고 있습니다. 새로운 방법과 수정된 방법은 [Audience Manager API 문서](https://bank.demdex.com/portal/swagger/index.html) 섹션을 참조하십시오.
...
* 때로 게시물이 시간 초과 오류로 인해 실패하던 문제를 수정했습니다.
* 중재 워크플로우 담당자가 Internet Explorer 11 브라우저에 표시되지 않는 문제를 해결했습니다.
* 사용자가 OR 연산자가 포함된 WordPress에 대한 의견 수렴 규칙을 만들지 못했던 문제를 수정했습니다.

See [Release Notes for Adobe Social](https://docs.adobe.com/content/help/ko-KR/social/using/home.html) for more information.
