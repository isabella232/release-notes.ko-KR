...
| 공지 | 추가 또는 업데이트 날짜 | 설명 |
| -----------| ---------- | ---------- |
| 통합 제품 도메인으로 마이그레이션 | 시행일: 2020년 5월 28일 | 2020년 1월에 시작된 Adobe Analytics용 통합 제품 도메인으로의 마이그레이션은 2020년 5월 28일에 완료되었습니다. Adobe Analytics이 모든 항목을 제거하려고 하는 동안 `omniture.com` 아키텍처에서 도메인 참조를 사용하면 `omniture.com` 를 타사 쿠키로 사용하는 경우도 있습니다. 전체 아키텍처 마이그레이션이 (곧) 완료되면 릴리스 정보를 통해 알려 드릴 예정이며 이 허용리스트 단계는 더 이상 필요하지 않습니다. [여기](https://helpx.adobe.com/kr/analytics/kb/adobe-ip-addresses.html) 허용해야 하는 권장 IP 주소 및 도메인 전체 목록입니다.<br>조직에서 타사 쿠키를 차단하는 경우 고객 지원 센터에 문의하여 Adobe Analytics에 다시 액세스하십시오. |
