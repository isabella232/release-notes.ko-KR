...
* **Dynamic Media에 추가된 3D 자산 지원**
   ...
   * AEM as a Cloud Service에서 [Dynamic Media에서 3D 자산 작업](https://docs.adobe.com/content/help/ko-KR/experience-manager-cloud-service/assets/dynamicmedia/assets-3d.html)을 참조하십시오.
...
| 2020년 6월 23일 | [처리 프로필](https://docs.adobe.com/content/help/en/experience-manager-learn/assets/configuring/processing-profiles.html) | 비디오 | 처리 프로필은 AEM as a Cloud Service에서 자산에 대해 만들 변환을 정의합니다. |
...
| 2020년 6월 23일 | [AEM as a Cloud Service 빌드 및 배포 디버깅](https://docs.adobe.com/content/help/en/experience-manager-learn/cloud-service/debugging/debugging-aem-as-a-cloud-service/build-and-deployment.html) | 문서 | AEM as a Cloud Service에 대한 빌드 및 배포를 디버깅하는 방법을 알아봅니다. |
| 2020년 6월 16일 | [로그를 사용하여 AEM as a Cloud Service 디버깅](https://docs.adobe.com/content/help/en/experience-manager-learn/cloud-service/debugging/debugging-aem-as-a-cloud-service/logs.html) | 문서 | 로그를 사용하여 AEM as a Cloud Service를 디버깅하는 방법을 알아봅니다. 로그는 AEM 응용 프로그램을 디버깅하기 위해 최전방 역할을 하지만 배포된 AEM 응용 프로그램에서 적절한 로깅에 의존합니다. |
...
| 기능 | 설명 |
| -----------| ---------- |
| GDPR 투명도 및 동의 프레임워크 | Advertising Cloud은 IAB의 투명도 및 동의 프레임워크에 대한 지원을 2020년 8월 15일까지 버전 2.0으로 업그레이드하고 있습니다. 이때, 고객측에서 변경할 필요가 없습니다.  각 고객을 위한 데이터 프로세서로서 Adobe Advertising Cloud은 고객이 Adobe 제품 및 서비스 사용에 적용되는 GDPR에 따른 의무를 이행할 수 있는 기능을 제공합니다. |
| Audience Manager 통합 | (Adobe Experience Platform 전용 Adobe Audience Manager 고객을 위한 베타) DSP은 광고주의 광고 참여 데이터가 포함된 실행 가능한 로그 파일을 Adobe Experience Platform을 통해 Audience Manager으로 보낼 수 있습니다. Audience Manager에서 광고주는 Audience Optimization 보고서와 같은 인사이트 도구에서 고급 세그멘테이션 및 사용을 위해 데이터를 사용할 수 있습니다. |
| 사람 기반 상호 장치 그래프 타깃팅 | 셀프 서비스 DSP 고객은 새로운 캠페인에서 사용자 기반의 타깃팅 및 빈도 관리를 위해 디바이스 그래프(Adobe Device Co-op 또는 LiveRamp)를 사용할 수 있습니다. 따라서 광고업체는 자사 디바이스를 통해 고객에게 도달하지만 노출 횟수를 제한할 수 있습니다. 또한 이제 배치 간 크로스 디바이스 타깃팅 설정의 일괄 편집이 베타 버전으로 제공됩니다. 계정에 대한 베타 버전을 활성화하려면 Adobe 계정 관리자에게 문의하십시오. |
| 사이트 타깃팅 | &quot;블랙리스트&quot; 및 &quot;화이트리스트&quot;가 보다 공정한 용어 &quot;차단된(사이트)&quot; 및 &quot;타깃팅된(사이트)&quot;로 이름이 변경되었습니다. |
| Flashtalking 광고 | 이제 DSP에서는 단일 광고 생성 및 대량 제3자 광고 생성 워크플로우를 통해 모든 Flashtalking 광고 태그에 대한 클릭 매크로를 자동으로 삽입합니다. |
| 보고서 | (Adobe Experience Platform 고객만을 위한 베타) Advertising Cloud Search의 새로운 크로스 채널 보고서는 DSP 배치 및 검색 캠페인에 대한 일별 주요 지표와 함께 DSP 및 검색 전체의 미디어 지출 내역을 통일적으로 볼 수 있도록 합니다. |
